                ...
                Forms\Components\Repeater::make('pagos')
                    ...
                    ->schema([
                        ...
                        Forms\Components\TextInput::make('monto_recibido')
                            ...
                            ->numeric()
                            ->reactive()
                            ->live(debounce: 800) // Cambiado de live() a live(onBlur: true) para mayor flexibilidad al escribir
                        ...
                        Forms\Components\TextInput::make('cambio')
                            ...
                            ->live(debounce: 800)                    // para que se refresque cuando cambie estado
    ...
    public function registrarPago(): void
    {
        ...
        try {
            DB::transaction(function () use ($factura, $data) {
                //
                // 1) Validación de montos (Sin cambios)
                //
                ...
                //
                // 2) Registro de cada Pago y cálculo de cambio (Sin cambios)
                //
                ...
                //
                // 3) Asignación de folio y estado (LÓGICA CORREGIDA)
                //
                if (! empty($data['usar_cai'])) {
                    // A. Obtener el CAI correcto y seguro
                    $cai = \App\Models\Cai::obtenerCaiSeguro($factura->empresa_id)
                        ?? throw new \Exception('No se encontró un rango de facturación válido y disponible.');

                    // B. Calcular el siguiente número de forma robusta
                    $siguienteNumero = $cai->numero_actual + 1;

                    // C. Validar y corregir si el 'numero_actual' estaba mal inicializado
                    //    (p.ej. 0 cuando el rango empieza en 2)
                    if ($siguienteNumero < $cai->rango_inicial) {
                        $siguienteNumero = $cai->rango_inicial;
                    }

                    // D. Validar que no se exceda el rango final
                    if ($siguienteNumero > $cai->rango_final) {
                        throw new \Exception("El rango de facturación para el CAI seleccionado se ha agotado.");
                    }

                    // E. Formatear y asignar el número de factura
                    $folioPadded   = str_pad($siguienteNumero, 8, '0', STR_PAD_LEFT);
                    ...
                    // F. Actualizar el CAI con el número que acabamos de usar
                    $cai->numero_actual = $siguienteNumero;
                    $cai->save();

                } else {
                    // Lógica para orden de compra (sin cambios)
                ...
                //
                // 4) Asignación de empleado (Sin cambios)
                //
