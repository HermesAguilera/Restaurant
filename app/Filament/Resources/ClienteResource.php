                        ...
                        ->schema([
                            Forms\Components\TextInput::make('persona.dni')
                                ...
                                ->unique(
                                    table: Persona::class,
                                    column: 'dni',
                                    // FIX 1: Ignora correctamente el ID de la persona asociada
                                    modifyRuleUsing: function (Unique $rule, callable $get) {
                                        $clienteId = $get('id');
                                        $personaId = Cliente::find($clienteId)?->persona_id;
                                        if ($personaId) {
                                            return $rule->ignore($personaId);
                                        }
                                        return $rule;
                                    }
                                ),
                ...
                ->schema([
                    ...
                    Forms\Components\Grid::make(2)
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('persona.telefono')
                                ->label('Teléfono')
                                ->rules(['nullable', 'string', 'max:20', 'regex:/^(?!0+$).*/'])
                                ->unique(
                                    table: Persona::class,
                                    column: 'telefono',
                                    // FIX 2: Mismo arreglo que el DNI, aplicado al teléfono
                                    modifyRuleUsing: function (Unique $rule, callable $get) {
                                        $clienteId = $get('id');
                                        $personaId = Cliente::find($clienteId)?->persona_id;
                                        if ($personaId) {
