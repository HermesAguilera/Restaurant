...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // --- 2. Crear roles para la empresa "GRUPO B" ---
        $this->command->info('Creando roles para ' . $empresa->nombre . '...');

        // Rol Root para GRUPO B
        $roleRoot = Role::firstOrCreate(
            [
                'name' => 'root',
                'empresa_id' => $empresa->id // <-- Asociamos a la empresa
            ],
            ['guard_name' => 'web']
        );
        $roleRoot->givePermissionTo(Permission::all()); // Asignar todos los permisos

        // Rol Admin para GRUPO B
        ...
        $roleAdmin->givePermissionTo(Permission::all()); // Asignar todos los permisos


        // --- 3. Crear el "Consumidor Final" (como Persona y Cliente) ---
        ...
        // --- 8. Asignar rol ---
        $user->assignRole($roleRoot); // <-- Asignamos el objeto rol que ya creamos
