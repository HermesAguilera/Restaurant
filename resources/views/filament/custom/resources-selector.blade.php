@php
    ...
    $customResourceNames = [
        ...
        'adelanto-salarials' => 'Adelanto salarial',
    ];
    ...
    foreach ($routes as $route) {
        ...
        if ($name && Str::startsWith($name, 'filament.') && Str::contains($name, '.resources.') && Str::endsWith($name, '.index')) {
            ...
            if ($resourceName === 'adelanto-salarials') {
                $displayName = 'Adelanto salarial';
            }
    ...
    $availableResources = collect($resourceRoutes)
        ->filter(function ($resource, $name) use ($user) {
            ...
            if (Str::contains($name, 'adelanto')) {
                $permissionName = 'nominas_ver';
            }
