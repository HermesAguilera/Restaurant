        ...
        <x-filament::card>
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="font-medium">{{ $record->nombre_cliente ?? 'Consumidor Final' }}</p>
                </div>
                <div>
                    ...
                    <tbody>
                        @foreach ($record->detalles as $detalle)
                            <tr class="border-b last:border-0">
                                ...
                                <td class="px-6 py-3 text-right">L. {{ number_format($detalle->precio_unitario, 2) }}</td>
                                <td class="px-6 py-3 text-right">{{ number_format($detalle->descuento_aplicado ?? 0, 2) }}%</td>
                                <td class="px-6 py-3 text-right">{{ number_format($detalle->isv_aplicado ?? 0, 2) }}%</td>
                                <td class="px-6 py-3 text-right">L. {{ number_format($detalle->sub_total, 2) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
